...
```{r}
plotfitted = function(results, rowInd, bgobj, lib_adj, legloc = 'topleft', 
    returnModel = FALSE, ...){
    require(splines)
...
```{r}
plotfitted(results_sorted, 242, gbg, libadjust, pch=19, col="#00000050", legloc='bottomleft')
```

and Figure 3b:

```{r}
plotfitted(results_sorted, 328, gbg, libadjust, pch=19, col="#00000050")
```
...
Finally, we'll repeat the RIN analysis using average coverage instead of FPKM as the expression measurement. We will need another Ballgown object for this, available [here](http://files.figshare.com/1625417/cov.rda). This code makes Figure 6d. Transcripts are downsampled to make the plot somewhat readable.

```{r, warning=FALSE}
load('cov.rda')
cov_small = subset(cov, "UseThisDup==1", genomesubset=FALSE) 
rrcov = stattest(cov_small, feature="transcript", meas="cov", 
    timecourse=TRUE, covariate="RIN", adjustvars="population")
# test same transcripts as were tested before:
rrcov_hiexpr = subset(rrcov, id %in% rin_results_hiexpr$id)
rrcov_hiexpr$qval = p.adjust(rrcov_hiexpr$pval, 'fdr')
sum(as.character(rrcov_hiexpr$id) != as.character(rin_results_hiexpr$id)) # expect 0 (sanity check)
