...
NSAMPLES=20
FOLDCHANGE=6
FOLDERNAME=$MAINDIR/forrealfinal_NBp0_FINALPAPER
PERCENTDE=0.1
...
EOF
    qsub -cwd -l mf=20G,h_vmem=5G -m e -M acfrazee+tophatNB@gmail.com $FOLDERNAME/tophat_${sample}.sh
done
...
EOF
qsub -l mf=10G,h_vmem=3G -pe local 4 -m e -M acfrazee+cufflinksNB@gmail.com $FOLDERNAME/cufflinks_$sample.sh
done
...
EOF
qsub -l mf=10G,h_vmem=3G -pe local 4 -M acfrazee+tablemakerNB@gmail.com $FOLDERNAME/ballgown_$sample.sh 
done
